#!/bin/bash
# Comprehensive startup script for Library Management System
# Starts frontend, backend, and full monitoring stack with proper networking
...
# Handle Ctrl+C (compatible with different shells)
trap cleanup INT 2>/dev/null || true
...
wait_for_services() {
    ...
    # Check Library App
    echo -e "${GRAY}   Checking Library App...${NC}"
    local attempt=1
    while [ $attempt -le $max_attempts ]; do
        if curl -f "http://localhost:8080/actuator/health" >/dev/null 2>&1; then
            echo -e "${GREEN}   ✅ Library App is healthy!${NC}"
            break
        fi
        if [ $attempt -eq $max_attempts ]; then
            echo -e "${YELLOW}   ⚠️  Library App health check timed out${NC}"
            break
        fi
        sleep 2
        attempt=$((attempt + 1))
    ...
    # Check Prometheus
    echo -e "${GRAY}   Checking Prometheus...${NC}"
    attempt=1
    while [ $attempt -le $max_attempts ]; do
        ...
        if [ $attempt -eq $max_attempts ]; then
            echo -e "${YELLOW}   ⚠️  Prometheus health check timed out${NC}"
            break
        fi
        sleep 2
        attempt=$((attempt + 1))
    ...
    # Check Grafana
    echo -e "${GRAY}   Checking Grafana...${NC}"
    attempt=1
    while [ $attempt -le $max_attempts ]; do
        ...
        if [ $attempt -eq $max_attempts ]; then
            echo -e "${YELLOW}   ⚠️  Grafana health check timed out${NC}"
            break
        fi
        sleep 2
        attempt=$((attempt + 1))
    ...
    # Check Zipkin
    echo -e "${GRAY}   Checking Zipkin...${NC}"
    attempt=1
    while [ $attempt -le $max_attempts ]; do
        if curl -f "http://localhost:9411/health" >/dev/null 2>&1; then
            echo -e "${GREEN}   ✅ Zipkin is healthy!${NC}"
            break
        fi
        if [ $attempt -eq $max_attempts ]; then
            echo -e "${YELLOW}   ⚠️  Zipkin health check timed out${NC}"
            break
        fi
        sleep 2
        attempt=$((attempt + 1))
